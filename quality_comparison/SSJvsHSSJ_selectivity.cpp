...
int main() {
cout << "#testing selectivity in aggregate" << endl;
double filter_selectivity_parameters[] = {0.5, 0.2, 0.1, 0.05, 0.02, 0.01};
for(double filter_selectivity_parameter : filter_selectivity_parameters) {
    ...
    auto h2_weighted = [&aggregate_f] (double C) -> double {return aggregate_f(0,0,C);};
    ...
    auto rand_filter = [&filter_selectivity_parameter] (double X, double Y) -> bool {
				int* Z;
				Z=(int*)&Y;
				return (((*Z)*1103515245+12345)%((int)round(1/filter_selectivity_parameter)))==0;
			    };
    ...
    for(int i_s : sampling_methods_used) {
        ...
        cout << filter_selectivity_parameter << "," << m << ",\""<< sample_types[i_s] << "\", \"" << filter_types[i_f] << "\",";
