...
int main() {
    ...
    //COMPUTE TRUE AGGREGATES GIVEN THAT THEY ARE INDEPENDENT OF B
    ...
    int nruns_values[] = {1000};
    int m_values[] = {10000};
    for(int nruns : nruns_values)
    for(int m_value : m_values) {
   ...
   //(*MISSING:  SSJ, US-Join, WS-Join (naive filtered)
   //and HSSJ and HWS-Join (naive filtered, expensive!) at m=10000*) 
	    if(i_f == 0) continue;
	    if(i_f == 1) continue;
	    if(i_f == 2) //Heuristic methods (naive filtered)
		    if(i_s != 1 && i_s != 3) continue;
            int progress_width = 5;
            for(int run_i=0; run_i<nruns; run_i++) {
                if(floor(progress_width*(run_i+1)/(double)nruns) > floor(progress_width*(run_i)/(double)nruns)) {
                    ...
                    if(progress_width == n_bars)
                        ...
                        cout << "] " << round(100*run_i/(double)nruns) << "%" << endl;
                }
